...
BEGIN
    ...
    ELSIF (lower(JN) = 'n') THEN
        RETURN 'Percentage';
    ELSE
        p_log(proc_name, 'E', 'Ja / Neen (aantal/percentage) kan niet vertaald worden. Is value niet geinitialiseer? ' || JN);
        RETURN '(zie log)';
    END IF;
EXCEPTION
...
BEGIN
    ...
    ELSIF (lower(JN) = 'n') THEN
        RETURN 'Neen';
    ELSE
        p_log(proc_name, 'E', 'Ja / Neen kan niet vertaald worden. Is een value niet geinitialiseerd? (NULL veroorzaakt problemen bij Cognos) ' || JN);
        RETURN '(zie log)';
    END IF;
EXCEPTION
...
BEGIN
    ...
    v_resultaat := APEX_MAIL.SEND(p_to => 'dirk.vermeylen@hpe.com',
        p_from => 'dirk.vermeylen@hpe.com',
        p_subj => 'MOW Dataroom Information',
        p_body => 'Tekst body',
        p_body_html => v_p_body_html
...
IS
    ...
    begin
        ...
        if v_laatst_bijgewerkt = v_no_data then
            ...
            p_cijfers2csv(f_indicatorfiche_id);   
        end if;
        RETURN v_laatst_bijgewerkt;       
    EXCEPTION
        WHEN OTHERS THEN
    ...
    PROCEDURE ip_send_mail (p_body_txt varchar2, p_nieuwe_indicator_vlag varchar2)
        ...
        v_resultaat number;
        v_to varchar2(4000) := 'bart.vanherbruggen@mow.vlaanderen.be,jurgen.steenhaut@mow.vlaanderen.be,dirk.vermeylen@hpe.com'; 
        v_to_nieuw varchar2(4000) := ',filip.heyvaert@hpe.com,anthony.vandevelde@zorg-en-gezondheid.be'; 
        -- v_to varchar2(4000) := 'dirk.vermeylen@hpe.com';   
        -- v_to_nieuw varchar2(4000) := ''; 
        v_cc varchar2(1024) := 'carla.boerenkamp@hpe.com';
        -- v_cc varchar2(1024) := ''; 
...
IS
    ...
    SELECT indicatorfiche_id,
        ...
        meetfrequentie,
        tijdvenster,
        to_char(fiche_laatst_bijgewerkt,'YYYY-MM-DD') fiche_bijgewerkt,
    ...
    type fiche_record is record (
        ...
        r_fiche_bijgewerkt varchar2(50),
...
utl_file.put_line(f,'<FicheBijgewerkt><![CDATA[' || fiche.r_fiche_bijgewerkt || ']]></FicheBijgewerkt>');
...
CREATE OR REPLACE PROCEDURE  "P_CIJFERS2CSV" (p_indic_id indicatorfiche.indicatorfiche_id%TYPE)
IS
    -- This procedure allows to extract cijferrecords for a specific indicatorfiche into html format.
    proc_name logtbl.evt_proc%TYPE := 'p_cijfers2csv';
    err_msg varchar2(255);
...
BEGIN
    ...
    SELECT f_referentielabel(meeteenheid), meetfrequentie, f_jn_aantal(aantal_percentage) 
        ...
        FROM indicatorfiche
        WHERE indicatorfiche_id = p_indic_id;
    v_filename := 'cijferscsv_' || lpad(p_indic_id, 2, '0') || '.csv';
    f := utl_file.fopen('DATAROOM_LOG', v_filename, 'w');
    v_cols := t_res(NULL);
    v_cols.extend;
    v_cols(ind) := v_meeteenheid;
    ...
    -- First print Table Header
    for j in 1..ind-1 loop
        v_hdr := v_hdr || v_cols(j) || ';';
    end loop;
    utl_file.put_line(f, v_hdr);
    -- Then print Rows.
    if v_res.count > 0 THEN
        for i in v_res.first..v_res.last loop
            v_hdr := '';
    ...
    if v_res.count > 0 THEN
        for i in v_res.first..v_res.last loop
            utl_file.put_line(f,'v_res(i)' || CHR(13) || CHR(10));
        end loop;
    END IF;
    utl_file.fclose(f);
    p_log(proc_name, 'I', 'End Application');

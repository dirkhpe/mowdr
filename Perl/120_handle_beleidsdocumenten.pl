...
sub handle_rec($$) {
	my ($lvl, $titel) = @_;
	my @fields = qw (titel parent_id toc doc_id);
	if ($lvl == ($prev_lvl + 1)) {
		...
		my $nr_toc = @toc_cnt - 1;
		for (my $cnt = $lvl + 1; $cnt <= $nr_toc; $cnt++) {
			pop @toc_cnt;
		}
		if ($lvl > 0) {
...
$query = "SELECT Document, L1, L2, L3, L4, L5, `Indicator/Metriek`
          FROM beleidsdocumenten
		  ORDER BY ID";
$ref = do_select($dbs, $query);
foreach my $record (@$ref) {
	my $document = $$record{Document} || "";
	my $l1       = $$record{L1}       || "";
	my $l2       = $$record{L2}       || "";
	my $l3       = $$record{L3}       || "";
	my $l4       = $$record{L4}       || "";
	my $l5       = $$record{L5}       || "";
	my $indic    = $$record{'Indicator/Metriek'} ||"";
	if (length($document) > 0) {
		$lvl = 0;
		handle_rec($lvl, $document);
	} elsif (length($l1) > 0) {
		$lvl = 1;
		handle_rec($lvl, $l1);
	} elsif (length($l2) > 0) {
		...
		$log->error("Empty record found, skipping record.");
	}
}

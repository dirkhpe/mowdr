...
my @fields = qw (indicator_fiche_id periode percentage actief hoofdvervoerswijze);
my $actief = "J";
my $indicator_fiche_id = $fiches{'verdeling van personen (scholieren en studenten) volgens hoofdvervoerswijze woon-schoolverkeer'};
$query = "SELECT d.vervoerswijze, procent, jaartal
          FROM `indicator verdeling personen hoofdvervoerswijze woon-school` i,
		       `dimensie vervoerswijze` d

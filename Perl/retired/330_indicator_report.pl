...
my @fields = qw (indicator_fiche_id periode percentage actief hoofdvervoerswijze afstandsklasse);
my $actief = "J";
my $indicator_fiche_id = $fiches{'verdeling van verplaatsingen volgens hoofdvervoerswijze en afstandsklasse'};
$query = "SELECT d.vervoerswijze, procent, jaartal, a.afstandsklasse
          FROM `indicator verplaatsingen volgens hoofdvervoersw en afstandskl` i,
		       `dimensie vervoerswijze` d, `dimensie afstandsklasse` a
		  WHERE i.vervoerswijze = d.id
		    AND i.afstandsklasse = a.id";
$ref = do_select($dbs, $query);
foreach my $record (@$ref) {
	my $hoofdvervoerswijze = $$record{'vervoerswijze'};
	my $periode = $$record{'jaartal'};
	my $percentage = $$record{'procent'};
	$hoofdvervoerswijze = $dim_els{$dimensies{'verplaatsing - hoofdvervoerswijze'} . $hoofdvervoerswijze};
	my $afstandsklasse = $$record{'afstandsklasse'};
	$afstandsklasse = $dim_els{$dimensies{'verplaatsing - afstandsklasse'} . $afstandsklasse};
